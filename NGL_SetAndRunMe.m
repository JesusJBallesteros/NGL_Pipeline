...
KSpath      = 'C:\Kilosort_2.0';     % Absolute path to kilosort, as it can change among PCs 
...
%     opt.disconnectedCh          = [];       % Array of disconnected channels. TO DO: which ordering? Helps to remove dead signals at start therefore from median removal and so on.
    opt.bin                     = true;     % Create a .bin file with the high-pass data, to be passed to Kilosort for spike sorting.
        ...
        opt.CAR                 = 1;        % Default: 1. Common Average Referencing (median) to remove fast-ample transients. 
    ...
    opt.kilosort                = false;    % Call to kilosort processing. !! NEEDS configfile saved under 'studyName\analysisCode\'
    ...
    opt.bombcell                = true;     % Run bombcell on the KS output, as previous step to manual curation. TODO: go over several KS outputs if existing.
