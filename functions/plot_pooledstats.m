...
if ~isfield(param,'pooled_plotcol'),param.pooled_plotcol = [0.0 0.0 0.0;
                                                            0.6 0.1 0.2];
end
...
param.poolraster.xtick = param.timelim(1):500:param.timelim(2); % time ticks
...
for a = 1:nalign
    ...
    poolneurons = cellfun(@(x) x*1000, poolneurons,'un',0);
    ...
    % Initialize figure
    fig = figure('visible', 'on'); % switch visibility
