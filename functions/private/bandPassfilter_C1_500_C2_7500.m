function bandPassfilter_C1_500_C2_7500(numChannels,filePathSingleChannels,filePathSingleChannelsFilt,HDF5chunkSize,bandPassfilterSettings)
fileNames = ls(filePathSingleChannels);
fileNames(1:2,:) = [];
for i = 1:numChannels 
    
    %estimate of total duration 0.33 s (per channel, using filtfilt) for dataset 220304
    filteredData = filter(bandPassfilterSettings,...
        double(... 
        h5read(...
