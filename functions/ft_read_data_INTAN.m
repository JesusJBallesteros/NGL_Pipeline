function [dat] = ft_read_data_INTAN(filename, varargin)
...
%         if any(contains(es_key,'lfp','IgnoreCase',true))
%             es_key = es_key(contains(es_key,'lfp','IgnoreCase',true));
        % MOD by Jesus 08.09.2022 to include Intan's raw data nomenclature
        if any(contains(es_key,'lowpass','IgnoreCase',true))
            es_key = es_key(contains(es_key,'acquisition','IgnoreCase',true)); 
        end
        % MOD by Jesus 08.09.2022 to include Intan's lowpass nomeclature
        if any(contains(es_key,'acquisition','IgnoreCase',true)) 
    ...
    for iCh=1:numel(chanindx)
        % TODO: function allows to load segments load([min_channel, min_sample],[max_channel, max_channel]) and one could subselect from there
        dat(iCh, :) = eseries.data.load([chanindx(iCh) begsample], [chanindx(iCh) endsample]); 
    end
