function Deuteron_PipelineWrapper(input, varargin)
...
% An empty 'trialdef' would tell that data shall be treated as continuous.
if opt.RetrieveEvents && opt.useexe
    ...
    trialdef = [];    
        % TODO (We need events to be recorded in SD card first, why is it failing?

elseif opt.RetrieveEvents && opt.usepar % Temporary solution? Working on Juan's paradigms.
    ...
    if opt.parsetrial
        opt.def = [];
            % Define event-code equivalency (Modify to match newest event convention)
            opt.def.itiOn       = 1;
            opt.def.stimOn      = 2;
            opt.def.rwd         = 5;
            opt.def.tutor       = 4;
        ...
        disp('Retrieving events from ''_par'' matlab file.')
