...
txt = sprintf('\n --> Subject %s, session %d out of %d: %s \n', ...
             input.subjects(input.run(2)).name, input.run(2), input.sessions(input.run(1)).nsessions, input.sessions(input.run(1)).list{input.run(2)});
fprintf(txt);
...
opt.PathRaw           = pwd;
opt.FolderProcDataMat = fullfile(input.processed, input.subjects(input.run(2)).name, input.sessions(input.run(1)).list{input.run(2)});
opt.behavFiles        = fullfile(input.bhvfolder, input.subjects(input.run(2)).name, input.sessions(input.run(1)).list{input.run(2)});
