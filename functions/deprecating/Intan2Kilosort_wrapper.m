...
% Reads the INTAN file, converts the values to microvolts, and saves it.
% No reshaping needed for fileperch or filepertype verions.
% No re-scaling for Kilosort is needed either.
%
% DEPENDENCIES
%
% INPUTS:
...
%              FolderProcDataMat: str. folder to save new files; 
%
% OUTPUT:
...
% Create complete HDF5 file to compile data matrix
h5create(fullfile(opt.FolderProcDataMat, [opt.SavFileName '.h5']), ...
        '/allChnMat', [opt.numChannels Inf], ...
        'ChunkSize', [1 opt.HDF5chunkSize], ...
        'Datatype', 'int16')

% Also create a bin file. It will process faster if the data is appended in chunks.  
...
% Total number of samples per channel is 'out.num_samples'
out.ChunkStart = 1:opt.StpSz:out.num_samples-mod(out.num_samples,opt.StpSz);
