...
if strcmp(artifacts,'noartifacts')
    no_artifacts = 1;
end

if (nargin > 1)
        file = {files.name}.';
        path = unique({files.folder}.');
end
...
for nf=1:nfiles
    ...
    while (filesize - ftell(fid) > 0)
        ...
        timestamp_seconds = double(timestamp) / double(sample_rate);
