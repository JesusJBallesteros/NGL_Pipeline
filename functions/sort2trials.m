...
for c=1:nclus 
    ...
    for a=1:size(trialdef,2)
        ...
        % for each trial
        for i=1:ntrial 
            % curr_unit = str2double(spike.label(j));
            idx = spike.timestamp{1,c} >= trialdef{2,a}(i,1) & ... % index for spiketimes btw trial strat
                  spike.timestamp{1,c} <  trialdef{2,a}(i,2);      % and trial end
            neurons.(opt.alignto{1,a}){c,1}{i,1} = spike.timestamp{1,c}(idx) - trialdef{2,a}(i,3); % and relativize time to the given alignment point
        end
    end
end
