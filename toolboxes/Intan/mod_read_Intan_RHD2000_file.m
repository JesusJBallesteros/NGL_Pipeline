function [INTAN_hdr] = mod_read_Intan_RHD2000_file(filename, varargin)
...
for f = 1:Nfiles
    ...
    if (data_present)
        ...
        if f == 1
            ...
            t_amplifier = zeros(1, num_amplifier_samples, Nfiles);
            amplifier_data = zeros(num_amplifier_channels, num_amplifier_samples, Nfiles);
            aux_input_data = zeros(num_aux_input_channels, num_aux_input_samples, Nfiles);
            supply_voltage_data = zeros(num_supply_voltage_channels, num_supply_voltage_samples, Nfiles);
            temp_sensor_data = zeros(num_temp_sensor_channels, num_supply_voltage_samples, Nfiles);
            board_adc_data = zeros(num_board_adc_channels, num_board_adc_samples, Nfiles);
            board_dig_in_data = zeros(num_board_dig_in_channels, num_board_dig_in_samples, Nfiles);
            board_dig_in_raw = zeros(1, num_board_dig_in_samples, Nfiles);
            board_dig_out_data = zeros(num_board_dig_out_channels, num_board_dig_out_samples, Nfiles);
            board_dig_out_raw = zeros(1, num_board_dig_out_samples, Nfiles);
        end

        % Read sampled data from file.
        fprintf(1, 'Reading data from file %d of %d ...\n', f, Nfiles);
        ...
        for i=1:num_data_blocks
            ...
            || (data_file_main_version_number > 1))
                t_amplifier(1,amplifier_index:(amplifier_index + num_samples_per_data_block - 1),f) = fread(fid, num_samples_per_data_block, 'int32');
            else
                t_amplifier(1,amplifier_index:(amplifier_index + num_samples_per_data_block - 1),f) = fread(fid, num_samples_per_data_block, 'uint32');
            end
            if (num_amplifier_channels > 0)
                amplifier_data(:, amplifier_index:(amplifier_index + num_samples_per_data_block - 1),f) = fread(fid, [num_samples_per_data_block, num_amplifier_channels], 'uint16')';
            end
            if (num_aux_input_channels > 0)
                aux_input_data(:, aux_input_index:(aux_input_index + (num_samples_per_data_block / 4) - 1),f) = fread(fid, [(num_samples_per_data_block / 4), num_aux_input_channels], 'uint16')';
            end
            if (num_supply_voltage_channels > 0)
                supply_voltage_data(:, supply_voltage_index,f) = fread(fid, [1, num_supply_voltage_channels], 'uint16')';
            end
            if (num_temp_sensor_channels > 0)
                temp_sensor_data(:, supply_voltage_index,f) = fread(fid, [1, num_temp_sensor_channels], 'int16')';
            end
            if (num_board_adc_channels > 0)
                board_adc_data(:, board_adc_index:(board_adc_index + num_samples_per_data_block - 1),f) = fread(fid, [num_samples_per_data_block, num_board_adc_channels], 'uint16')';
            end
            if (num_board_dig_in_channels > 0)
                board_dig_in_raw(1,board_dig_in_index:(board_dig_in_index + num_samples_per_data_block - 1),f) = fread(fid, num_samples_per_data_block, 'uint16');
            end
            if (num_board_dig_out_channels > 0)
                board_dig_out_raw(1,board_dig_out_index:(board_dig_out_index + num_samples_per_data_block - 1),f) = fread(fid, num_samples_per_data_block, 'uint16');
            end
...
if f > 1
    % Concatenate all data into expected matrices
    t_amplifier = reshape(t_amplifier, 1,[]);
    amplifier_data = reshape(amplifier_data, num_amplifier_channels,[]);
    aux_input_data = reshape(aux_input_data, num_aux_input_channels, []);
    supply_voltage_data = reshape(supply_voltage_data, num_supply_voltage_channels, []);
    temp_sensor_data = reshape(temp_sensor_data, num_temp_sensor_channels, []);
    board_adc_data = reshape(board_adc_data, num_board_adc_channels, []);
    board_dig_in_data = reshape(board_dig_in_data, num_board_dig_in_channels, []);
    board_dig_in_raw = reshape(board_dig_in_raw, 1, []);
    board_dig_out_data = reshape(board_dig_out_data, num_board_dig_out_channels, []);
    board_dig_out_raw = reshape(board_dig_out_raw, 1, []);
end
...
% assignin('base', variable_name, string(variable));
% 
% return;