...
if exist('spikeTimes_samples','var')
    ...
    ephysData.ephys_sample_rate = 32000;
...
% load raw waveforms 
rawWaveforms.average = readNPY([fullfile(path.savePath, 'templates._bc_rawWaveforms.npy')]);
rawWaveforms.peakChan = readNPY([fullfile(path.savePath, 'templates._bc_rawWaveformPeakChannels.npy')]);
...
[uniqueTemplates, ~, ephysData.spike_times_samples, ephysData.spike_templates, ephysData.template_amplitudes, ...
    ...
    param.ephys_sample_rate, param.saveSpikes_withoutDuplicates, path.savePath, param.recomputeDuplicateSpikes);

% load other gui stuffs 
if ~exist('forGUI', 'var') || ~isempty(dir([path.savePath, filesep, 'templates.qualityMetricDetailsforGUI.mat']))
    load([path.savePath, filesep, 'templates.qualityMetricDetailsforGUI.mat'])
else
    forGUI = struct;
