function [rawWaveformsFull, rawWaveformsPeakChan, signalToNoiseRatio] = bc_extractRawWaveformsFast(param, spikeTimes_samples, ...
    spikeTemplates, path)
% Get raw waveforms for all templates
% Inputs
% param with:
    ...
    % detrendWaveform: boolean, whether to detrend spikes or not
...
reExtract = param.reextractRaw;

%% Extract raw waveforms
if param.extractRaw % Extract wfs
    rawWaveformFolder = dir(fullfile(path.savePath, 'templates._bc_rawWaveforms.npy'));

if ~isempty(rawWaveformFolder) && reExtract == 0 % simply load it
    rawWaveformsFull = readNPY(fullfile(path.ephysKilosortPath, 'templates._bc_rawWaveforms.npy'));
    rawWaveformsPeakChan = readNPY(fullfile(path.ephysKilosortPath, 'templates._bc_rawWaveformPeakChannels.npy'));
...
%     BatchSize       = 5000;
    ...
    fprintf('\n Extracting raw waveforms from %s ...', path.rawFile)
    
    %% Open binary file
    fid = fopen(path.rawFile, 'r');
    ...
    for iCluster = 1:nClust
        ...
        % Clear current cluster raw spikes
    ...
    if ~isfolder(path.savePath)
        mkdir(path.savePath)
    end
    writeNPY(rawWaveformsFull, fullfile(path.savePath, 'templates._bc_rawWaveforms.npy'))
    writeNPY(rawWaveformsPeakChan, fullfile(path.savePath, 'templates._bc_rawWaveformPeakChannels.npy'))

    % Save mean raw waveform baseline average (for signal-to-noise calculation)
    ...
    writeNPY(average_baseline_cat, fullfile(path.savePath, 'templates._bc_baselineNoiseAmplitude.npy'))
    writeNPY(average_baseline_idx_cat, fullfile(path.savePath, 'templates._bc_baselineNoiseAmplitudeIndex.npy'))
end
...
if ~isempty(fullfile(path.savePath, 'templates._bc_baselineNoiseAmplitude.npy'))
    
    average_baseline_cat = readNPY(fullfile(path.savePath, 'templates._bc_baselineNoiseAmplitude.npy'));
    average_baseline_idx_cat = readNPY(fullfile(path.savePath, 'templates._bc_baselineNoiseAmplitudeIndex.npy'));
