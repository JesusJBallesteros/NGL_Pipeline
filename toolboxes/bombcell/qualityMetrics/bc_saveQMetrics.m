function qMetric = bc_saveQMetrics(param, qMetric, forGUI, path)
...
%
% Modified by Jesus 29/11/2023 (no parquet files)

if ~exist(path.savePath, 'dir')
    mkdir(fullfile(path.savePath))
end

% save parameters
if ~istable(param)
    if isempty(path.ephysKilosortPath)
        param.ephysKilosortPath = 'NaN';
    end
%     parquetwrite([fullfile(path.savePath, '_bc_parameters._bc_qMetrics.parquet')], struct2table(param))
end

% save quality metrics
if param.saveMatFileForGUI
    save(fullfile(path.savePath, 'templates.qualityMetricDetailsforGUI.mat'), 'forGUI', '-v7.3')
end

% save fraction refractory period violations for all different tauR times
% parquetwrite([fullfile(path.savePath, 'templates._bc_fractionRefractoryPeriodViolationsPerTauR.parquet')], array2table(qMetric.fractionRPVs))
...
% violations for each unit's estimated tauR make sure everything is a double first
...
% parquetwrite([fullfile(path.savePath, 'templates._bc_qMetrics.parquet')], qMetricTable)

% overwrite qMetric with the table, to be consistent with it for next steps of the pipeline
...
if isfield(param,'saveAsTSV') % ensure back-compatibility if users have a previous version of param
    ...
    if param.saveAsTSV == 1 
        ...
        if isfield(path,'ephysKilosortPath') && strcmp(path.ephysKilosortPath, 'NaN') == 0
            saveTSV_path = path.ephysKilosortPath;
        else
