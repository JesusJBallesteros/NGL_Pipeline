function [unitType, unitType_string] = bc_getQualityUnitType(param, qMetric, path)
...
if (nargin < 3 || isempty(path.savePath)) && param.unitType_for_phy == 1
    savePath = pwd;
        ...
        if param.unitType_for_phy == 1
            if isfield(param, 'saveAsTSV') % ensure back-compatibility if users have a previous version of param
                if param.saveAsTSV == 1
                    ...
                    if isfield(path, 'ephysKilosortPath')
