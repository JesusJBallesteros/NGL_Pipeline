...
After closing Phy (manual curation complete):

```matlab
cd(input.analysisCode)
postPhy_param();     % set NGL02 options
cd(toolbox)
NGL02_postPhy        % processes spike + LFP data into lab-standard variables
```

**Key `opt` fields for NGL02** (set in `NGL_SetAndRunMe` or `postPhy_param`):

| Field | Default | Description |
|---|---|---|
| `doSpikething` | `true` | Build single-unit spike variables |
| `doLFPthing` | `true` | Build LFP variables |
| `trialparsed` | `false` | Use trial-parsed FT file instead of continuous |
| `artifdet` | `false` | LFP artifact detection |
| `spectrogram` | `false` | Multitaper TF analysis |
| `FLIP` | `false` | vFLIP laminar power analysis |

> **Important:** Phy must have been run and its output saved before NGL02 can proceed. NGL02 reads the `cluster_group.tsv` and related files written by Phy from `kilosort/4/`.

---

## 12. Troubleshooting

### "Option 'X' is not recognised by default_opt() and will be ignored"

You set a field in `opt` that doesn't exist in `default_opt.m`. Check for typos. If you intentionally added a new field, add it to `default_opt.m` first.

### "More INTAN files than number of channels specified"

`opt.numChannels` is set lower than the actual number of `amp*.dat` files. Either increase `opt.numChannels` or confirm which channels were active during the recording.

### Kilosort fails with a Python error

- Confirm the KS4 Python environment path in `NGL_machineConfig` (`cfg.KSpythonExe`).
- Verify Kilosort 4 is installed: activate the env and run `python -c "import kilosort"`.
- Ensure no other `pyenv` call has been made earlier in the session (MATLAB only supports one Python environment per session).

### "opt.lowpass must be a scalar between 0 and 9500 Hz"

INTAN's hardware low-pass ceiling is ~9500 Hz. If you genuinely want no filtering, set `opt.lowpass = []`.

### NWB and `.h5` conflict

`opt.doNWB = true` and H5 output cannot be used in the same MATLAB session due to a DLL collision. Restart MATLAB between runs if both are needed.

### FieldTrip `ft_redefinetrial` fails with mismatched trial boundaries

Trial definitions (`trialdef`) are stored in **milliseconds** and converted to samples inside `MAT2FieldTrip`. If you see sample-index errors, ensure `opt.dwnsmplRate` was correctly set (or left as `[]` for auto) during the LFP processing step. The conversion formula is:

```matlab
cfg.trl = round(trialdef{2,i} / 1000 * fs_lfp);
```

where `fs_lfp` is derived from `data.time{1}` — not from `opt.dwnsmplRate` directly.

### `downsampleVolt` assert: "not integer multiple of original data"

The downsample target must divide evenly into the raw sample rate. With a 30 kHz INTAN system, valid targets include 937.5 Hz (30000/32), 1000 Hz (30000/30), 1500 Hz (30000/20), etc. The default 937.5 Hz is the standard choice.
...
Use `git diff --name-only HEAD -- ':!toolboxes/*'` to confirm staged changes are only in project files. Toolbox folders should be excluded via `.gitignore`.

---

*For IKN hard-disk standard details see: `gitlab.ruhr-uni-bochum.de/ikn/howto/-/wikis/Neurophysiology/hard-disk-data-structure`*
