...
assert(isfield(opt.popDyn,'pcaConditions') && iscell(opt.popDyn.pcaConditions) && ...
       ~isempty(opt.popDyn.pcaConditions) && all(cellfun(@ischar, opt.popDyn.pcaConditions)), ...
    'NGL:invalidOption', ['opt.popDyn.pcaConditions must be a non-empty cell of chars. ', ...
                          'Each entry is one condition field name or ''X vs Y'' comparison; ', ...
                          'default {''allInitiated''}.']);
...
assert(ischar(fp.cacheDir), 'NGL:invalidOption', 'opt.fireRatePlot.cacheDir must be a char (empty for default <input.analysis>/cache/firepools).');

% Cross-subject population PCA plotter (NGL04_PCA).
assert(isstruct(opt.pcaPlot), 'NGL:invalidOption', 'opt.pcaPlot must be a struct.');
pp = opt.pcaPlot;
assert(isnumeric(pp.interval) && numel(pp.interval) == 2 && pp.interval(1) < pp.interval(2), ...
    'NGL:invalidOption', 'opt.pcaPlot.interval must be [pre post] ms with pre<post.');
assert(isnumeric(pp.binSize_ms) && isscalar(pp.binSize_ms) && pp.binSize_ms > 0, ...
    'NGL:invalidOption', 'opt.pcaPlot.binSize_ms must be a positive scalar (ms).');
assert(isnumeric(pp.stepSz_ms) && isscalar(pp.stepSz_ms) && pp.stepSz_ms > 0 && pp.stepSz_ms <= pp.binSize_ms, ...
    'NGL:invalidOption', 'opt.pcaPlot.stepSz_ms must be a positive scalar (ms) <= binSize_ms.');
assert(isnumeric(pp.smoothSigma_s) && isscalar(pp.smoothSigma_s) && pp.smoothSigma_s >= 0, ...
    'NGL:invalidOption', 'opt.pcaPlot.smoothSigma_s must be a non-negative scalar (s).');
assert(isnumeric(pp.nComponents) && isscalar(pp.nComponents) && pp.nComponents >= 2, ...
    'NGL:invalidOption', 'opt.pcaPlot.nComponents must be a scalar >= 2.');
assert(isnumeric(pp.nBootstrap) && isscalar(pp.nBootstrap) && pp.nBootstrap >= 0, ...
    'NGL:invalidOption', 'opt.pcaPlot.nBootstrap must be a non-negative scalar (0 disables CI).');
assert(isempty(pp.rngSeed) || (isnumeric(pp.rngSeed) && isscalar(pp.rngSeed)), ...
    'NGL:invalidOption', 'opt.pcaPlot.rngSeed must be [] or a scalar integer.');
assert(isnumeric(pp.sessionAlpha) && isscalar(pp.sessionAlpha) && pp.sessionAlpha >= 0 && pp.sessionAlpha <= 1, ...
    'NGL:invalidOption', 'opt.pcaPlot.sessionAlpha must be in [0,1].');
assert(isnumeric(pp.ciAlpha) && isscalar(pp.ciAlpha) && pp.ciAlpha >= 0 && pp.ciAlpha <= 1, ...
    'NGL:invalidOption', 'opt.pcaPlot.ciAlpha must be in [0,1].');
assert(isnumeric(pp.ciStride) && isscalar(pp.ciStride) && pp.ciStride >= 1, ...
    'NGL:invalidOption', 'opt.pcaPlot.ciStride must be a positive scalar.');
assert(iscell(pp.variants) && ~isempty(pp.variants) && all(cellfun(@ischar, pp.variants)) && ...
    all(ismember(pp.variants, {'singleTrials','ciTube'})), ...
    'NGL:invalidOption', 'opt.pcaPlot.variants must be a non-empty cell of {''singleTrials'',''ciTube''}.');
assert(ischar(pp.outDir), 'NGL:invalidOption', 'opt.pcaPlot.outDir must be a char (empty for default).');
